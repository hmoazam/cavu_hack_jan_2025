...
# MAGIC %pip install -U -qqqq mlflow-skinny langchain==0.2.16 langgraph-checkpoint==1.0.12 langchain_core langchain-community==0.2.16 langgraph==0.2.16 pydantic databricks_langchain==0.1.1 
...
#  Uncomment if you have created uc functions and have added them in the config.yml file
...
# Fill in with your VS endpoint name and VS index name
VECTOR_INDEX_NAME = "kyra_wulffert.default.wikipedia_vector_index"
...
retriever_tool = create_retriever_tool(
  ...
  description="Find films related to topic", # TODO: Fill in your retriever's description to help the LLM choose this tool
)
...
tools = [] # TODO: Remove if you have tools defined in uc and loaded in the cell above
...
for event in agent.stream({"messages": [{"role": "user", "content": "Find movies that address societal issues like racism or income inequality"}]}):
    print(event)
